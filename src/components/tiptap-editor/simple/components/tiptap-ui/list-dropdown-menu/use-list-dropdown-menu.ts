...
// --- Icons ---
import { RiListCheck3, RiListOrdered, RiListUnordered } from "@remixicon/react";
...
export function useListDropdownMenu(config?: UseListDropdownMenuConfig) {
  ...
  const filteredLists = useMemo(() => {
    const options = getFilteredListOptions(types);
  ...
  return {
    ...
    types,
    filteredLists,
    label: t("toolbar.list.label"),
    Icon: activeList ? listIcons[activeList.type] : RiListUnordered,
  };
}
