...
import separatorIcon from "remixicon/icons/Editor/separator.svg?raw";
...
export default class Delimiter {
  ...
  get availableDelimiterStyles() {
    return this._config.styleOptions
      ? Delimiter.DELIMITER_STYLES.filter((style) =>
          this._config.styleOptions.includes(style)
        )
  ...
  get userDefaultDelimiterStyle() {
    if (this._config.defaultStyle) {
      ...
      console.warn(
        "(ง'̀-'́)ง Delimiter Tool: the default style specified is invalid"
      );
    }
  ...
  get availableLineWidths() {
    return this._config.lineWidthOptions
      ? Delimiter.SUPPORTED_LINE_WIDTHS.filter((width) =>
          this._config.lineWidthOptions.includes(width)
        )
  ...
  get userDefaultLineWidth() {
    if (this._config.defaultLineWidth) {
      ...
      // eslint-disable-next-line no-console
      console.warn(
        "(ง'̀-'́)ง Delimiter Tool: the default line width specified is invalid"
      );
    }
  ...
  get availableLineThickness() {
    return this._config.lineThicknessOptions
      ? Delimiter.LINE_THICKNESS.filter((thickness) =>
          this._config.lineThicknessOptions.includes(thickness)
        )
  ...
  get userDefaultLineThickness() {
    if (this._config.defaultLineThickness) {
      ...
      // eslint-disable-next-line no-console
      console.warn(
        "(ง'̀-'́)ง Delimiter Tool: the default line thickness specified is invalid"
      );
    }
  ...
  _normalizeData(data) {
    ...
    newData.lineWidth =
      parseInt(data.lineWidth, 10) || this.userDefaultLineWidth;
    newData.lineThickness =
      parseInt(data.lineThickness, 10) || this.userDefaultLineThickness;
    return newData;
  }
  ...
  get currentDelimiterStyle() {
    let delimiterStyle = this.availableDelimiterStyles.find(
      (style) => style === this._data.style
    );
    if (!delimiterStyle) {
  ...
  get currentLineWidth() {
    let lineWidth = this.availableLineWidths.find(
      (width) => width === this._data.lineWidth
    );
    if (!lineWidth) {
  ...
  _createChildElement() {
    ...
    if (this.currentDelimiterStyle in delimiterMap) {
      ...
      child.classList.add(
        this._CSS.wrapperForThickness(this.currentLineThickness)
      );
    }
